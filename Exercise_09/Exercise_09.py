...
class about_Mercury:
    ...
    def show_result_precession(self):
        ...
        for i in range(self.n):
            pl.plot([0, self.p_x[i]], [0, self.p_y[i]], 'r')
        pl.grid()
        pl.show()
    #时间与进动角度关系图
    def show_result_theta_versus_time(self):
        ...
        pl.plot(self.p_t, self.theta, 'ob')
        pl.plot([0, self.p_t[-1] * 1.05], [0, self.theta[-1] * 1.05], 'r')
        pl.show()
    ...
    def show_result_precession_rate_versus_alpha(self):
        ...
        pl.plot(self.alpha_1, self.dtheta_dt, 'ob')
        pl.plot([0, self.alpha_1[-1] * 1.05], [0, self.dtheta_dt[-1] * 1.05], 'r')
        pl.show()
        

start = about_Mercury(0.001, 1)
start.calculate()
